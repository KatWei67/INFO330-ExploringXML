...
<xsl:template match="/pokedex">
    Strong attackers: 
    <xsl:apply-templates select="count(pokemon[stats/attack/text() > 150])" />
    Strong defenders:
    <xsl:apply-templates select="count(pokemon[stats/defense/text() > 150])" />
    Fast:
    <xsl:apply-templates select="count(pokemon[stats/speed/text() > 150])" />
</xsl:template>
...
<xsl:template match="/pokedex">
  <html>
  <body>
  <h2>Strong Attackers: <xsl:value-of select="count(pokemon[stats/attack/text() > 150])" /></h2>
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
      <th>Type</th>
    </tr>
    <xsl:apply-templates select="pokemon[stats/attack/text() > 150]" />
  </table>
  <h2>Strong Defenders: <xsl:value-of select="count(pokemon[stats/defense/text() > 150])" /></h2>
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
      <th>Type</th>
    </tr>
    <xsl:apply-templates select="pokemon[stats/defense/text() > 150]" />
  </table>
  <h2>Fast: <xsl:value-of select="count(pokemon[stats/speed/text() > 150])" /></h2>
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
      <th>Type</th>
    </tr>
    <xsl:apply-templates select="pokemon[stats/speed/text() > 150]" />
  </table>
  </body>
  </html>
</xsl:template>
